...
void TcasKO33(int Cur_Vertical_Sep,int High_Confidence,int Two_of_Three_Reports_Valid,int Own_Tracked_Alt,int Own_Tracked_Alt_Rate,int Other_Tracked_Alt,int Alt_Layer_Value,int Up_Separation,int Down_Separation,int Other_RAC,int Other_Capability,int Climb_Inhibit)
	{ __CPROVER_assume((Cur_Vertical_Sep==1016) &&(High_Confidence==1)&&(Two_of_Three_Reports_Valid==0) && (Own_Tracked_Alt == 712)&&(Own_Tracked_Alt_Rate == 426)&&(Other_Tracked_Alt == 0)&&(Alt_Layer_Value == 0)&&(Up_Separation == 361)&&(Down_Separation == 745)&&(Other_RAC == 0)&&(Other_Capability == 2)&&(Climb_Inhibit == 1));int Positive_RA_Alt_Thresh[4];
        int OLEV;
	int MAXALTDIFF;
	int MINSEP;
	int NOZCROSS;
