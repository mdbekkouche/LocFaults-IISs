...
void TcasKO33(int Cur_Vertical_Sep,int High_Confidence,int Two_of_Three_Reports_Valid,int Own_Tracked_Alt,int Own_Tracked_Alt_Rate,int Other_Tracked_Alt,int Alt_Layer_Value,int Up_Separation,int Down_Separation,int Other_RAC,int Other_Capability,int Climb_Inhibit)
	{ __CPROVER_assume((Cur_Vertical_Sep==860) &&(High_Confidence==1)&&(Two_of_Three_Reports_Valid==0) && (Own_Tracked_Alt == -1)&&(Own_Tracked_Alt_Rate == 485)&&(Other_Tracked_Alt == 422)&&(Alt_Layer_Value == 0)&&(Up_Separation == 330)&&(Down_Separation == 273)&&(Other_RAC == 0)&&(Other_Capability == 2)&&(Climb_Inhibit == 1));int Positive_RA_Alt_Thresh[4];
        int OLEV;
	int MAXALTDIFF;
	int MINSEP;
	int NOZCROSS;
