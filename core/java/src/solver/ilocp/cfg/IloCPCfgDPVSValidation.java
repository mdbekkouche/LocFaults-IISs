...
public class IloCPCfgDPVSValidation extends CfgSingleCspValidation {
	...
	public IloCPCfgDPVSValidation(String name) throws IloException {
		if (buildCFG()) {
			simplifyCFG();	
	...
	@Override
	protected IloCPCfgCsp createCSP() {
		if (VerboseLevel.TERSE.mayPrint()) {
			System.out.println("Creating solver(s):");
		}
		return new IloCPCfgCsp("Z3 (rational CSP)");		
	}
	...
	public boolean isFeasible() {
		...
		foundSolution = true;
		try {
			foundSolution = ((IloCPSolver)csp.solver).solver().propagate();
