...
import validation.strategies.cfg.localization.DFSdevieVisitor.TypeCondDeviated;
import validation.system.cfg.CfgCsp;
import validation.util.Type;

public class DFSdevieVisitorExtented extends DFSdevieVisitor {

	private CfgCsp cspcp;
	private CfgCsp csp;
	private boolean  currentCspIsLinear;
	private Stack<Boolean> stack;
	
	public void setCSP(CfgCsp csp, CfgCsp cspcp) {
		this.csp = csp;
		this.cspcp = cspcp;
		this.currentCspIsLinear=true;
	}
	...
	private void addCtrAndPref(LogicalExpression ctr, int startLine) {		
		...
		cspcp.addConstraint(ctr, ctrName);
		ctrInfos.put(ctrName, new CtrInfo(startLine, ctr));
		
		currentCspIsLinear = currentCspIsLinear && ctr.isLinear();
	...
	public void visit(EnsuresNode n) throws CFGVisitException, IloException {
		...
		if (size==0 && n.marked==-1){
			...
			if (Validation.solverCombination == Solver.SolverCombination.CPLEXCP_OPTIMIZER) {
				if (!currentCspIsLinear) solverName="CP OPTIMIZER"; 
				else solverName="CPLEX";
			}
			...
			if (solverName=="CPLEX"){
				...
				CPOPTIMIZERAllMCSlowerK allmcslowerk = new CPOPTIMIZERAllMCSlowerK(cspcp,ctrInfos,k,suspInsts);
				//currentCspIsLinear=true;
				// We add MCS computed
				allMCSProgramAss.add(allmcslowerk.arrListMCSes);
