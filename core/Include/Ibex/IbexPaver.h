...
#ifndef _IBEX_SOLVER_H
#define _IBEX_SOLVER_H

#include <iostream>
#include <vector>
#include <stack>
#include "IbexException.h"
#include "IbexEvaluator.h" // just for UnfeasibilityException (may be removed then)
#include "IbexCell.h"
...
class Precision : public Contractor {
 public:
  ...
  /** Copy constructor. */
  Precision(const Precision& p) : Operator(p.space), ceil(p.ceil) { }
 ...
 * It allows to apply alternatively a list of contractors and a bisector until the stack gets empty.
...
class Paver {
  ...
  /** Create a "standard" solver with a contractor and a bisector. 
   * The space of the solver is a reference to the space of the contractor in argument.*/
  ...
  long nb_boxes(int ctc) { return accepted[ctc].size(); }
  ...
  */
  ...
  /* true when only one contractor is used besides precision */
  bool one_ctc;
