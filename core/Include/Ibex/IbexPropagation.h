...
class Propagation : public Contractor {
 public:
   ...
   * \param vec - The list of contractors.
   ...
   * \return true if the box was <i>significantly</i> reduced, i.e., if at least one domain reduction has 
   * triggered propagation (i.e., has pushed some pair <i>(var,ctr)</i> back into the queue).
   * \throw #ibex::UnfeasibilityException - if inconsistency is detected. */ 
  ...
  /** Ratio (see \link Propagation(const vector<const Contractor*>&, Space&, REAL, bool) constructor \endlink for details). */
  const REAL ratio;

  /** Queue initialization mode (see \link Propagation(const vector<const Contractor*>&, Space&, REAL, bool) constructor \endlink for details).*/
