...
class Certification : public Contractor {
 public:
   ...
   * In case of succes, an UnfeasibilityException is thrown. 
  ...
  /** Copy constructor. */
  Certification(const Certification& c) : Operator(c.space), prover(c.prover->copy()), funnel(c.funnel->copy()), ceil(c.ceil) { }
